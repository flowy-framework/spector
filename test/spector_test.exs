defmodule SpectorTest do
  ...
  describe "validate keys" do
    test "known specs without types" do
      schema = %{name: %{}, context: %{}}
      spec = %{name: MyProducer, context: :ok}

      assert Spector.validate(spec, schema) == {:ok, spec}
    end
    ...
    test "unknown options" do
      ...
      assert Spector.validate(opts, schema) ==
               {:error,
                %ValidationError{
                  key: [:not_an_option1, :not_an_option2],
                  value: nil,
                  message:
                    "unknown specs [:not_an_option1, :not_an_option2], valid options are: [:an_option, :other_option]"
                }}
    end
  end
  ...
  describe "validate the schema itself before validating the options" do
    test "raise ArgumentError when invalid" do
      ...
      message = """
      invalid Spector schema. \
      Reason: invalid value for :type option: unknown type :foo.

      Available types: :any, :list, :non_empty_list, :map, :atom, \
    ...
    test "validate the keys recursively, if any" do
      ...
      invalid Spector schema. \
      Reason: \
      unknown specs [:unknown_schema_option], \
      valid options are: [:default, :type, :keys, :deprecated, \
      :doc, :required, :type_spec, :type_doc, :subsection] \
      (in options [:producers, :keys, :*, :keys, :module])\
      """

      assert_raise ArgumentError, message, fn ->
        Spector.validate(%{}, schema)
      end
    end
  end
  ...
  describe "default value" do
    test "is used when none is given" do
      ...
      assert Spector.validate(%{}, schema) == {:ok, %{context: :ok}}
    end

    test "is not used when one is given" do
      schema = %{context: %{default: :ok}}
      assert Spector.validate(%{context: :given}, schema) == {:ok, %{context: :given}}
    end
  ...
  describe ":required" do
    test "when present" do
      ...
      assert Spector.validate(opts, schema) == {:ok, opts}
    end
  ...
  describe ":doc" do
    test "valid documentation for key" do
      ...
      assert Spector.validate(%{}, schema) == {:ok, %{context: 1}}
    ...
    test "invalid documentation for key" do
      message = """
      invalid Spector schema. Reason: expected :doc option to match at least \
  ...
  describe ":deprecated" do
    ...
    test "warns when given" do
      ...
      assert capture_io(:stderr, fn ->
               assert Spector.validate(%{context: :ok}, schema) == {:ok, %{context: :ok}}
             end) =~ ":context option is deprecated. Use something else"
    end

    test "does not warn when not given" do
      schema = %{context: %{deprecated: "Use something else"}}
      assert Spector.validate(%{}, schema) == {:ok, %{}}
    end
  ...
  describe "type validation" do
    ...
    test "invalid positive integer" do
      ...
      assert Spector.validate(%{stages: 0}, schema) ==
               {:error,
                %ValidationError{
                  key: :stages,
                  value: 0,
      ...
      assert Spector.validate(%{stages: :an_atom}, schema) ==
               {:error,
                %ValidationError{
                  key: :stages,
                  value: :an_atom,
    ...
    test "invalid integer" do
      ...
      assert Spector.validate(%{min_demand: 1.5}, schema) ==
               {:error,
                %ValidationError{
                  key: :min_demand,
                  value: 1.5,
      ...
      assert Spector.validate(%{min_demand: :an_atom}, schema) ==
               {:error,
                %ValidationError{
                  key: :min_demand,
                  value: :an_atom,
    ...
    test "invalid non negative integer" do
      ...
      assert Spector.validate(%{min_demand: -1}, schema) ==
               {:error,
                %ValidationError{
                  key: :min_demand,
                  value: -1,
    ...
    test "invalid float" do
      ...
      assert Spector.validate(%{certainty: 1}, schema) ==
               {:error,
                %ValidationError{
                  key: :certainty,
                  value: 1,
      ...
      assert Spector.validate(%{certainty: :an_atom}, schema) ==
               {:error,
                %ValidationError{
                  key: :certainty,
                  value: :an_atom,
    ...
    test "invalid atom" do
      ...
      assert Spector.validate(%{name: 1}, schema) ==
               {:error,
                %ValidationError{
                  key: :name,
                  value: 1,
    ...
    test "invalid string" do
      ...
      assert Spector.validate(%{doc: :an_atom}, schema) ==
               {:error,
                %ValidationError{
                  key: :doc,
                  value: :an_atom,
    ...
    test "invalid boolean" do
      ...
      assert Spector.validate(%{required: :an_atom}, schema) ==
               {:error,
                %ValidationError{
                  key: :required,
                  value: :an_atom,
    ...
    test "valid {:or, subtypes} with compound subtypes" do
      ...
      opts = %{docs: "123"}
      assert Spector.validate(opts, schema) == {:ok, %{docs: 123}}
    end
    ...
    test "valid {:or, subtypes} with nested keyword lists" do
      schema = %{
        docs: %{
          type: {:or, [:boolean, list: %{enabled: %{type: :boolean}}]}
        }
      }
    ...
    test "{:custom, mod, fun, args} can also cast the value of an option" do
      ...
      opts = %{connections: "5"}
      assert {:ok, validated_opts} = Spector.validate(opts, schema)
    ...
    test "{:custom, mod, fun, args} enforces the returned value of the function" do
      ...
      message =
        "custom validation function SpectorTest.misbehaving_custom_validator/1 " <>
          "must return {:ok, value} or {:error, message}, got: :ok"

      assert_raise RuntimeError, message, fn ->
        assert Spector.validate(%{my_option: :whatever}, schema)
      end
    end
    ...
    test "{:list, subtype} with custom subtype" do
      ...
      assert Spector.validate(opts, schema) == {:ok, %{metadata: [123, 456]}}
    ...
    # for subtype <- [:list, :non_empty_list] do
    ...
    #   assert Spector.validate(valid_opts, schema) ==
    ...
    #   assert Spector.validate(invalid_opts, schema) == {
    #            :error,
    #            %Spector.ValidationError{
    #              key: :map_list,
    #              keys_path: [],
      ...
      assert Spector.validate(opts, schema) ==
               {:error,
                %Spector.ValidationError{
                  ...
                  key: [:unknown_key],
                  keys_path: [:map],
                  message: "unknown specs [:unknown_key], valid options are: [:key]",
                  value: nil
                }}
    end
      ...
      assert Spector.validate(opts, schema) ==
               {:error,
                %Spector.ValidationError{
                  ...
                  keys_path: [],
                  message:
                    "invalid value for :struct option: expected URI, got: %Spector{schema: %{}}",
                  value: %Spector{schema: %{}}
                }}
    end
    ...
    test "invalid {:struct, struct_name}" do
      ...
      assert_raise(
        ArgumentError,
        "invalid Spector schema. Reason: invalid value for :type option: invalid struct_name for :struct, expected atom, got \"123\" (in options [:struct])",
        fn ->
          Spector.validate(opts, schema)
        end
      )
    end
  end
  ...
  describe "nested options with predefined keys" do
        ...
        assert Spector.validate(opts, schema) ==
                 {:error,
                  %ValidationError{
                    ...
                    keys_path: [:processors],
                    message:
                      "unknown specs [:unknown_option1, :unknown_option2], valid options are: [:stages, :min_demand]"
                  }}
      end
    end
    ...
    for type <- [:list, :map] do
      test "empty default option with default values for #{type}" do
        ...
        assert Spector.validate(%{}, schema) == {:ok, %{processors: processors}}
      end
    end
  ...
  describe "nested options with custom keys" do
        ...
        assert Spector.validate(opts, schema) ==
                 {:error,
                  %ValidationError{
                    ...
                    message: "unknown specs [:unknown_option], valid options are: [:module, :arg]"
                  }}
      end
    end
    ...
    for type <- [:list, :map] do
      test "options with default values for #{type}" do
        ...
        {:ok, %{producers: validated_producers}} = Spector.validate(opts, schema)
    ...
    test "validate empty keys for :non_empty_list" do
      schema = %{
        producers: %{
          type: :non_empty_list,
          keys: %{
    ...
    for type <- [:list, :map] do
      test "default value for #{type}" do
        ...
        assert Spector.validate(opts, schema) == {:ok, %{batchers: empty}}
      end
    end
  end
  ...
  describe "validate!/2 (raising version)" do
    test "returns the direct options if the options are valid" do
      ...
      assert Spector.validate!(opts, schema) == opts
    end

    test "raises a Spector.ValidationError if the options are invalid" do
      schema = %{an_option: %{}, other_option: %{}}
      opts = %{an_option: 1, not_an_option1: 1, not_an_option2: 1}
      ...
      assert_raise Spector.ValidationError, message, fn ->
        Spector.validate!(opts, schema)
      end
    end
  end
  ...
  describe "option_typespec/1" do
    ...
    test "joins typespec correctly" do
      ...
      assert Spector.option_typespec(schema) ==
    ...
    test "all possible types" do
      cases = [
        {:any, quote(do: term())},
        {:list, quote(do: keyword())},
        {:non_empty_list, quote(do: keyword())},
        {:map, quote(do: map())},
        ...
        {{:list, {:list, []}}, quote(do: [keyword()])},
        {{:list, {:non_empty_list, []}}, quote(do: [keyword()])},
    ...
    #     assert clean_context_meta(Spector.option_typespec(schema)) ==
  ...
  @compile_time_wrapper Spector.new!(%{an_option: %{}})

  describe "wrapper struct" do
    @tag emi: true
    test "can be built from a valid schema" do
      valid_schema = %{an_option: %{}, other_option: %{}}
      assert %Spector{} = Spector.new!(valid_schema)

      invalid_schema = %{hello: :atom}

      assert_raise FunctionClauseError, fn ->
        Spector.new!(invalid_schema)
      end
    end

    test "will not be validated once built" do
      invalid_schema = %{"a_binary_key" => []}
      invalid_struct = %Spector{schema: invalid_schema}

      assert catch_error(Spector.validate(%{}, invalid_struct))
    end

    test "can be built at compile time" do
      assert {:ok, _} = Spector.validate(%{an_option: 1}, @compile_time_wrapper)
    end
  end
  ...
  defp opts_to_type(:list, opts), do: opts
